...
unique_categories = list(set([item for sublist in [cat.split('|') for cat in categories] for item in sublist]))
...
def generate_items():
    ...
    item_df = pd.read_csv('feature_repo/data/generated_amazon.csv')
    for item_id, item in item_df.iterrows():
        ...
        # Generate URLs
        img_link = f"https://raw.githubusercontent.com/RHEcosystemAppEng/rec-sys-app/refs/heads/main/generation/data/generated_images/item_{product_name.replace(' ', '%20')}.png"
...
def generate_interactions(users_df: pd.DataFrame, items_df: pd.DataFrame, num_interactions: int):
    ...
    for _ in range(num_interactions):
        ...
        if np.random.random() < 0.9 and user_prefs:  # 70% chance to select from preferred categories
...
if __name__ ==  '__main__':
    ...
    dummy_clip_feature_df.to_parquet('feature_repo/data/item_clip_features_embed.parquet', index=False)
