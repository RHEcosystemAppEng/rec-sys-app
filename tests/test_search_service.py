...
def search_service(mock_feature_store):
    with patch('service.search_by_text.AutoTokenizer') as mock_tokenizer, \
...
def test_search_by_text_empty_results(search_service, mock_feature_store):
    # Mock empty results
    mock_feature_store.retrieve_online_documents.return_value.to_df.return_value = pd.DataFrame(
        columns=['item_id', 'event_timestamp']
    )
    mock_feature_store.get_historical_features.return_value.to_df.return_value = pd.DataFrame(
