...
def _demographic_filtering(user_df: pd.DataFrame, item_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, user in user_df.iterrows():
        ...
        # Age-based filtering
        if user['age'] <= 25:
            user_items_df = user_items_df[user_items_df['category'].isin(['Electronics', 'Clothing', 'Sports'])]
        elif user['age'] >= 50:
            user_items_df = user_items_df[user_items_df['category'].isin(['Home', 'Books'])]
...
def _user_history(user_df: pd.DataFrame, item_df: pd.DataFrame, 
    ...
    for _, user in user_df.iterrows():
        ...
        if user['user_id'] in interacted_items.index:
            ...
            user_items = user_items[~user_items['item_id'].isin(interacted_items[user['user_id']])]
...
def _contextual_filters(user_df: pd.DataFrame, item_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if is_weekend:
        ...
        item_df = item_df[
            item_df['category'].isin(['Electronics']) |
            (item_df['popular'] == True)
        ]
    elif 18 <= current_hour <= 23:  # Evening hours
        item_df = item_df[
            item_df['category'].isin(['Books']) |
            (item_df['popular'] == True)
        ]
        
    return item_df

def filter_items(user_df: pd.DataFrame, item_df: pd.DataFrame, 
    ...
    context_filtered = _contextual_filters(user_df, history_filtered.drop(columns=['user_id']))
    
    # Merge user_ids back and ensure unique items per user
    final_result = history_filtered[['user_id']].drop_duplicates().merge(
        context_filtered, how='cross'
    )
