...
class EntityTower(nn.Module):
    def __init__(self, num_numerical: int= 1, num_of_categories: int = 1, d_model: int=64, text_embed_dim: int = 384, image_embed_dim: int = 384, dim_ratio: Dict = {'numeric': 1, 'categorical': 2, 'text': 7, 'image': 0}):
        super().__init__()
    ...
    def forward(self, numerical_features: Tensor, categorical_features: Tensor, text_features: Tensor, url_image: Tensor):
        ...
        if self.num_of_categories > 0:
            x_categorical = self.categorical_embed(categorical_features).squeeze(1) # n_samples, cat_dim
