...
class ClipEncoder:
    ...
    def encode_texts_and_images(self, texts: list[str], images: list[Image], batch_size: int = 32):
        ...
        return torch.nn.functional.normalize(combined_embeddings, p=2, dim=1).cpu().detach().numpy().tolist()
    ...
    def encode_images_batched_having_nones(self, images: list[Image], batch_size: int = 32):
        all_embeddings = []
        all_none_indices = []
